    ...
    public abstract class ConnexionUtil
    {
        ...
        public static IUser Login(string user, string password)
        {
            ...
            if (ValidateAttempts(user))
            {
                ...
                else if (aDManager.authenticate(user, password))
                {
                    ...
                    if (db.User.Include(u => u.Role).SingleOrDefault((u) => u.GUID.ToLower().Equals(aDManager.getUserAD(user).SamAccountName) && u.Role.HadesAccess) != null)
                    {
                        //Check Allowed in HADES (is in DB as User)
                        User u = db.User.SingleOrDefault((u) => u.GUID.ToLower().Equals(aDManager.getUserAD(user).SamAccountName));
                        db.Update(u);
                        u.Attempts = 0;
                        db.SaveChanges();
                        return u;
        ...
        private static bool ValidateAttempts(string user)
        {
            ...
            catch (Exception)
            {
                ...
                try
                {
                    User u = db.User.SingleOrDefault((a) => a.GUID.ToLower().Equals(aDManager.getUserAD(user)));
                    if (u == null)
                    {
                        throw new ForbiddenException();
                    }
